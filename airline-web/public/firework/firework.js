...
function updateGlobals(timeStep, lag) {
    ...
    if (store.state.config.autoLaunch) {
        if (fireworkEndTimestamp > Date.now()) {
            autoLaunchTime -= timeStep;
            if (autoLaunchTime <= 0) {
                autoLaunchTime = startSequence() * 1.25;
            }
        } else {
            setTimeout(() => { Ticker.stopListener(); }, 20000)
        }
    }
}
...
function colorSky(speed) {
    ...
    appNodes.canvasContainer.style.backgroundColor = `rgba(${currentSkyColor.r | 0}, ${currentSkyColor.g | 0}, ${currentSkyColor.b | 0}, 0.5)`;
}
...
//        // The sound toggle button is triggered on 'cstart', which doesn't seem to count as a full
